                            ...
                            <div class="header-right">
                                <ul class="actions">
                                    <li>
                                        ...
                                        @else
                                            <a href="#" data-toggle="modal" data-target="#pop-login" style="font-size: 30px;"><span><i class="fa fa-heart-o"></i></span></a>
                                        @endif;
...
@section('javascript')
    ...
    <script src="https://maps.google.com/maps/api/js?key=AIzaSyAFqAPWaxVQnJMkCBEHvlP1fIqevvgoN44?callback=initMap"
    ...
    <script type="text/javascript">
        ...
        function initMap() {
            ...
            map.Map(document.getElementById('map2'), {
                zoom: 12,
                center: map.LatLng(<?php echo $property_info['LATITUDE'] ?>,<?php echo $property_info['LONGITUDE'] ?>),
                mapTypeId: google.maps.MapTypeId.ROADMAP,
                styles: styles["hide"]
            });

            var beachMarker = map.Marker({
            ...
            myInterval = setInterval(function(){jQuery("#loading_div").css('display','block');get_fb(javascript_array,localStorage.getItem('test')); y++; parseInt(localStorage.setItem('test', y))}, 2000);
        }
        ...
        function plot_map(locations,amenity) {
            for (i = 0; i < locations.length; i++) {
                if (locations[i][1]) {
                    //console.log("{{ url('/') }}/images/am_images/" + amenity + ".png");
                    marker = map.Marker({
                        position: map.LatLng(locations[i][1], locations[i][2]),
                        map: map,
                        icon: "{{ url('/') }}/images/am_images/" + amenity + ".png"
                    });
